...
object KoreanHolidays {
    ...
    /**
     * 100년치(2024년 ~ 2125년) 대한민국 완벽한 법정공휴일, 음력 명절(설날 연휴 3일, 추석 연휴 3일, 부처님오신날) 및 대체공휴일 DB
     */
    ...
    private fun build100YearsHolidays(): Set<LocalDate> {
        ...
        for (year in 2024..2125) {
            ...
            // 2. 음력 명절 (설날 연휴 3일, 부처님오신날, 추석 연휴 3일) 및 대체공휴일
            try {
                ...
                // 추석 연휴 중 일요일 포함 시 또는 개천절 등 다른 공휴일 중복 시 대체공휴일
                if (c1.dayOfWeek == DayOfWeek.SUNDAY || c2.dayOfWeek == DayOfWeek.SUNDAY || c3.dayOfWeek == DayOfWeek.SUNDAY ||
                    c1 == LocalDate.of(year, 10, 3) || c2 == LocalDate.of(year, 10, 3) || c3 == LocalDate.of(year, 10, 3)) {
        ...
        // 3. 확정된 주요 선거일 추가
        set.add(LocalDate.of(2027, 3, 3))  // 2027년 제21대 대선
        set.add(LocalDate.of(2028, 4, 12)) // 2028년 제23대 총선
        set.add(LocalDate.of(2032, 3, 3))  // 2032년 제22대 대선
